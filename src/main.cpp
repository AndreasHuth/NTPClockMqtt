...
void reconnect() {
  ...
  while (!client.connected()) {
    ...
    if (client.connect(clientId.c_str())) {
      ...
      // ... and resubscribe
      client.subscribe(sub_action); client.loop();
      client.subscribe(sub_display_clear); client.loop();
      client.subscribe(sub_display_Brightness); client.loop();
      client.subscribe(sub_display_control); client.loop();

      client.subscribe(sub_alarm_minute); client.loop();
      client.subscribe(sub_alarm_hour); client.loop();
      client.subscribe(sub_alarm_on); client.loop();
      client.subscribe(sub_alarm_set); client.loop();
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if (strcmp(topic, sub_display_Brightness) == 0) {
      ...
      if (MQTTclockBrightness > 7) MQTTclockBrightness = 7;
  ...
  if (strcmp(topic, sub_alarm_hour) == 0) {
      ...
      if (MQTTclockAlarmHours > 23) MQTTclockAlarmHours = 0;
 ...
 if (strcmp(topic, sub_alarm_minute) == 0) {
     ...
     if (MQTTclockAlarmMinutes > 59) MQTTclockAlarmMinutes = 0;
...
void eraseConfig(void) {
  ...
  EEPROM.begin(512);
  for (signed int i = cfgStart ; i < sizeof(cfg) ; i++) {
    EEPROM.write(i, 0);
  }
  delay(200);
  EEPROM.commit();
...
void loop()
{
    ...
    if ((last_read == 0) || (abs(now - last_read) > INTERVAL2))
    {
        ...
        // Apply it.
        if (!MQTTsleepMode){
            if (!setAlarm)   tm1637.point(flash);
        ...
        //Serial.print (cur_hour_value); Serial.print (" : "); Serial.println(cur_min_value);
        ...
        Serial.println(brightness_setpoint_neopixel);*/
